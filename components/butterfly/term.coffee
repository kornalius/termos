...
class Terminal extends EventEmitter
  ...
  constructor: (@parent, @out, @ctl = (->), @scrollback = 1000000, @buffSize = 100000) ->
    EventEmitter @

    # for k in _.keys(chalk.prototype.constructor)
      # if !Terminal.prototype[k]?
        # Terminal.prototype[k] = chalk.prototype.constructor[k]

    # Global elements
    @document = @parent.ownerDocument
    # @body = @document.getElementsByTagName('body')[0]
    @body = @parent
    @forceWidth = @body.getAttribute('data-force-unicode-width') is 'yes'
    ...
    @rows = Math.floor(@body.clientHeight / @charSize.height)
    ...
    @cursor =
      ...
      fill: (x, y, w, h, ch = null, attr = null) =>
        while y < y + h
          line = @screen[y + @shift]
          chars = line.chars
          line.dirty = true
          i = x
          while i < x + w
            line[i] = @cloneAttr attr or line[i].attr, ch or line[i].ch
            i++
          y++
        @refresh()
        return @cursor
      ...
      eraseRectangle: (x, y, w, h) =>
        @eraseRectangle([y, x, y + h - 1, x + w - 1])
        @refresh()
        return @cursor
  ...
  equalAttr: (a, b) ->
    # Not testing char
    (a.bg is b.bg and a.fg is b.fg and
     a.bold is b.bold and a.underline is b.underline and a.italic is b.italic and
     a.blink is b.blink and a.inverse is b.inverse and a.invisible is b.invisible and
     a.font is b.font)
      ...
      for i in [0..@cols - 1]
        ...
        unless @equalAttr data, attr
          ...
          unless @equalAttr data, @defAttr
            ...
            # font
            classes.push "font#{data.font}" if data.font
        ...
        out += "<span class=\"" + (
          if @cursorState then "reverse-video " else "") + "cursor\">" if i is x
          ...
          switch ch
            ...
            when ">"
              ...
              if ch == " "
                ...
                skipnext = true
                # out += "<span style=\"display: inline-block; width: #{2 * @charSize.width}px\">#{ch}</span>"
                out += "<span>#{ch}</span>"
              else
                # out += "<span style=\"display: inline-block; width: #{@charSize.width}px\">#{ch}</span>"
                out += "<span>#{ch}</span>"
  ...
  resize: (x=null, y=null) ->
    ...
    @rows = y or Math.floor(@body.clientHeight / @charSize.height)
    # px = window.innerHeight % @charSize.height
    # @body.style['padding-bottom'] = "#{px}px"
